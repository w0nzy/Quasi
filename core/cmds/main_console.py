...
class Console(Cmd):
    ...
    def show_clients(self):
        for num,ip in DataBase.all_client_num_tags.items():
            ...
            aes_key = parse_list(DataBase.all_client_aes_credentials[ip],index=0)
            aes_iv = parse_list(DataBase.all_client_aes_credentials[ip],index=1)
            c = Client(client_sock=sock,client_aes_key=aes_key,client_aes_iv=aes_iv)
            if c.is_online():
    ...
    def do_listen(self,line: str):
        ...
        parser.new_arguments("-p",name="port",is_required=True,typing="int", help_doc="Sunucunun bind edileceği port numarası")
        p = parser.parse_args()
        if not p is False:
            ...
            if not p is False:
                ...
                try:
                    ...
                    if ip:
                        ...
                        port = DataBase.all_client_addrs[ip]
                        hostname = DataBase.all_client_hostnames[ip]
                        key = parse_list(DataBase.all_client_aes_credentials[ip],index=0)
                        iv = parse_list(DataBase.all_client_aes_credentials[ip],index=1)
                        print_status("Key: ",key)
                        print_status("Iv: ",iv)
                        stat = Client(client_sock=sock,client_aes_key=key,client_aes_iv=iv).is_online()
                        if stat:
                            c = CliCmd(
                                sock=sock,
                                host=ip,
                                port=port,
                                aes_key=key,
                                aes_iv=iv,hostname=hostname
                            )
                            c.cmdloop()
                        else:
                            print_failure("%s(%s) istemcisi Online değil" % (ip,hostname))
                except KeyError:
    ...
    def do_connect(self,line: str):
        start()
    def do_build_info(self,line: str):
        print_slice("Yapımcı: \t\t ",Build.author)
        print_slice("Versiyon: \t\t ",Build.version)
        print_slice("Yapı ID: \t\t ",Build.build_id)
        print_slice("Yapı Açıklaması \t ",Build.description)
        print_slice("Desteklenen OS \t ",Build.supported_os)
